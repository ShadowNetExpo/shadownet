...
(function(){
  ...
  var battleExitChannel = null, postBattleActive = false;
  ...
  function handleEnded(row){
    ...
    showWinner(row);
    // KEEP split-screen visible; show exit button after crown fades.
    postBattleActive = true;
    setupBattleExitChannel(row.id || activeBattle.id);
    setTimeout(function(){
      if (postBattleActive) showPostBattleUI();
    }, 6500);
  }

  function setupBattleExitChannel(battleId){
    if (battleExitChannel) try{battleExitChannel.unsubscribe();}catch(e){}
    battleExitChannel = window.sb.channel('battle-exit-' + battleId);
    battleExitChannel.on('broadcast', { event: 'exit_battle' }, function(){
      exitBattleMode(true);
    }).subscribe();
  }

  function showPostBattleUI(){
    if (!postBattleActive) return;
    // Persistent banner indicating post-battle state
    var container = document.querySelector('.live-video-wrap');
    if (container && !document.getElementById('lbPostBanner')) {
      var banner = document.createElement('div');
      banner.id = 'lbPostBanner';
      banner.className = 'lb-post-banner';
      banner.textContent = '\u2694\uFE0F BATALLA TERMINADA';
      container.appendChild(banner);
    }
    // Exit button - any host can click to bring everyone back to solo live
    var myId = window.currentUser && window.currentUser.id;
    var iAmHost = activeBattle && (myId === activeBattle.host_a || myId === activeBattle.host_b);
    if (!iAmHost) return;
    if (document.getElementById('lbExitBtn')) return;
    var btn = document.createElement('button');
    btn.id = 'lbExitBtn';
    btn.className = 'lb-exit-btn visible';
    btn.innerHTML = '\u21A9 Volver a mi live';
    btn.onclick = function(){
      if (!confirm('\u00bfSalir del modo batalla?\nAmbos creators volver\u00e1n a su live solo.')) return;
      // Notify peer first, then locally exit
      if (battleExitChannel) {
        try { battleExitChannel.send({type:'broadcast', event:'exit_battle', payload:{from: myId}}); } catch(e){}
      }
  ...
  function exitBattleMode(fromPeer){
    postBattleActive = false;
    restoreSplitScreen();
    var btn = document.getElementById('lbExitBtn');
    if (btn) btn.remove();
    var banner = document.getElementById('lbPostBanner');
    if (banner) banner.remove();
    if (battleExitChannel) {
      try { battleExitChannel.unsubscribe(); } catch(e){}
      battleExitChannel = null;
    }
    activeBattle = null;
  }
